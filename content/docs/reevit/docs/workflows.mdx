...
Automate actions triggered by payment events. Configure workflows in your Reevit dashboard to automatically send emails, SMS, notifications, or trigger webhooks when specific payment events occur.
...
2. **Check Conditions**: Reevit evaluates workflow conditions (amount, currency, method, etc.)
...
1. Go to **Workflows** in your Reevit dashboard
...
Connect your own messaging providers. Reevit uses a "Bring Your Own Key" (BYOK) model, so you maintain direct relationships with providers while benefiting from unified workflow automation.
...
```bash
curl -X POST https://api.reevit.com/v1/workflows/integrations \
...
```bash
curl https://api.reevit.com/v1/workflows/integrations \
  -H "X-Reevit-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123"
```
...
```bash
curl -X POST https://api.reevit.com/v1/workflows/integrations/integ_abc123/default \
  -H "X-Reevit-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123"
```
...
```bash
curl -X POST https://api.reevit.com/v1/workflows/templates \
  -H "X-Reevit-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123" \
...
```bash
curl https://api.reevit.com/v1/workflows/templates \
  -H "X-Reevit-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123"
```
...
1. Go to **Workflows**
2. Click **Create Workflow**
3. Select trigger from Reevit events
...
```bash
curl -X POST https://api.reevit.com/v1/workflows/rules \
  -H "X-Reevit-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123" \
...
```bash
curl https://api.reevit.com/v1/workflows/rules \
  -H "X-Reevit-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123"
```
...
```bash
# Disable
curl -X POST https://api.reevit.com/v1/workflows/rules/wf_abc123/disable \
  -H "X-Reevit-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123"

# Enable
curl -X POST https://api.reevit.com/v1/workflows/rules/wf_abc123/enable \
  -H "X-Reevit-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123"
```
...
```bash
curl "https://api.reevit.com/v1/workflows/executions?limit=20" \
  -H "X-Reevit-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123"
```
...
```bash
curl https://api.reevit.com/v1/workflows/executions/exec_123/actions \
  -H "X-Reevit-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123"
```
