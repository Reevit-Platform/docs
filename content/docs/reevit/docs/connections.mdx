---
title: Connections
description: Connect your own PSP accounts to Reevit (Bring Your Own Key)
icon: Link
---
...
Connect your existing payment service provider (PSP) accounts to Reevit. This "Bring Your Own Key" (BYOK) model means you maintain direct relationships with providers while benefiting from Reevit's unified API, smart routing, and failover capabilities.
...
A **connection** represents your credentials for a specific PSP account. Think of it as linking your existing Paystack, Hubtel, or Stripe account to Reevit.
...
Reevit supports major payment providers across Africa and globally:
...
- **Let Reevit route**: We'll automatically use the best one
...
| `provider` | PSP name | Tells Reevit which provider API to use |
...
| `capabilities` | What this connection supports | Helps Reevit route intelligently |
...
**Important**: 
- Configure your Reevit webhook URL in Stripe Dashboard
...
```bash
curl -X POST https://api.reevit.com/v1/connections/test \
  -H "X-Reevit-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123" \
...
```bash
curl https://api.reevit.com/v1/connections \
  -H "X-Reevit-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123"
```
...
```bash
curl https://api.reevit.com/v1/connections/paystack_live \
  -H "X-Reevit-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123"
```
...
Labels help Reevit route intelligently:

```bash
curl -X PATCH https://api.reevit.com/v1/connections/paystack_live/labels \
  -H "X-Reevit-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123" \
  -d '{
...
```bash
curl -X PATCH https://api.reevit.com/v1/connections/paystack_live/status \
  -H "X-Reevit-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123" \
  -d '{
    "status": "inactive"
...
Each connection tracks performance metrics to help Reevit route intelligently.
...
**Low health score (&lt;0.90)**: Connection may have issues, Reevit will prefer alternatives
...
```bash
curl https://api.reevit.com/v1/connections/paystack_live/audit \
  -H "X-Reevit-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123"
```
...
Connections work seamlessly with Reevit's routing system.

### Automatic Routing

Reevit automatically selects the best connection based on:
...
```bash
curl -X POST https://api.reevit.com/v1/routing-rules \
  -H "X-Reevit-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123" \
  -d '{
...
**Don't disable** just because you have backups - Reevit uses multiple connections for better success rates.
...
4. **Health scores**: Lower health scores may cause Reevit to prefer alternatives
