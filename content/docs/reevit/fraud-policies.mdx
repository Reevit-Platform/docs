...
```bash
curl https://api.reevit.io/v1/policies/fraud \
...
```bash
curl -X POST https://api.reevit.io/v1/policies/fraud \
  -H "X-Reevit-Key: pfk_live_xxx.secret" \
  -H "X-Org-Id: org_123" \
...
```bash
curl https://api.reevit.io/v1/policies/fraud/audit \
  -H "X-Reevit-Key: pfk_live_xxx.secret" \
  -H "X-Org-Id: org_123"
```
...
```bash
curl https://api.reevit.io/v1/policies/retry \
  -H "X-Reevit-Key: pfk_live_xxx.secret" \
  -H "X-Org-Id: org_123"
```
...
```bash
curl https://api.reevit.io/v1/policies/retry/list \
  -H "X-Reevit-Key: pfk_live_xxx.secret" \
  -H "X-Org-Id: org_123"
```
...
```bash
curl -X DELETE "https://api.reevit.io/v1/policies/retry?plan_id=premium" \
  -H "X-Reevit-Key: pfk_live_xxx.secret" \
  -H "X-Org-Id: org_123"
```
