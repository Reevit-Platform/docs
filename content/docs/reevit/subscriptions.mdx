---
title: Subscriptions
description: Manage recurring billing and automated dunning
icon: Repeat
---

import { CreditCardIcon, GlobeIcon as WebhookIcon, WorkflowSquare01Icon as WorkflowIcon, Link01Icon as LinkIcon, RepeatIcon } from "@hugeicons/core-free-icons"
...
```bash
curl -X POST https://api.reevit.io/v1/subscriptions \
...
<Callout type="info">
Reevit automatically injects the correlation metadata needed for PSP webhooks. Use `metadata` for your own fields like plan tier or signup source.
</Callout>
...
| `method` | Payment method | `mobile_money`, `card`, `bank_transfer` (aliases: `momo`, `bank`) | How customer will be charged |
...
```bash
curl "https://api.reevit.io/v1/subscriptions?status=active" \
  -H "X-Reevit-Key: pfk_live_xxx.secret" \
  -H "X-Org-Id: org_123"
```
...
```bash
curl https://api.reevit.io/v1/subscriptions/sub_abc123 \
  -H "X-Reevit-Key: pfk_live_xxx.secret" \
  -H "X-Org-Id: org_123"
```
...
```bash
curl -X PATCH https://api.reevit.io/v1/subscriptions/sub_abc123 \
  -H "X-Reevit-Key: pfk_live_xxx.secret" \
  -H "X-Org-Id: org_123" \
...
```bash
curl -X POST https://api.reevit.io/v1/subscriptions/sub_abc123/cancel \
  -H "X-Reevit-Key: pfk_live_xxx.secret" \
  -H "X-Org-Id: org_123" \
...
```bash
curl -X POST https://api.reevit.io/v1/subscriptions/sub_abc123/resume \
  -H "X-Reevit-Key: pfk_live_xxx.secret" \
  -H "X-Org-Id: org_123" \
...
```bash
curl "https://api.reevit.io/v1/invoices?subscription_id=sub_abc123" \
  -H "X-Reevit-Key: pfk_live_xxx.secret" \
  -H "X-Org-Id: org_123"
```
...
```bash
curl https://api.reevit.io/v1/invoices/inv_xyz789 \
  -H "X-Reevit-Key: pfk_live_xxx.secret" \
  -H "X-Org-Id: org_123"
```
...
```bash
curl -X POST https://api.reevit.io/v1/invoices/inv_xyz789/retry \
  -H "X-Reevit-Key: pfk_live_xxx.secret" \
  -H "X-Org-Id: org_123" \
...
```bash
curl -X POST https://api.reevit.io/v1/policies/retry \
  -H "X-Reevit-Key: pfk_live_xxx.secret" \
  -H "X-Org-Id: org_123" \
...
Subscriptions raise events inside Reevit, and they drive the dashboard and
your notification preferences. **They are not delivered to your webhook
endpoint.**

<Callout type="warn">
There are no outbound `subscription.*` or `invoice.*` webhooks. Reevit
delivers `payment.*` and `refund.*` events only — see
[Event Types](/docs/reevit/webhooks#event-types). Subscribing to the names below produces
an endpoint that receives nothing.

To react to a renewal, handle the `payment.updated` event for the renewal
charge and correlate it through the metadata you set when creating it.
</Callout>

The events below exist in the dashboard and in notification preferences.
...
Because there is no `subscription.*` delivery, drive access from the payment
that actually reaches your endpoint. The discriminator is `event`, not `type`.

```javascript
// A renewal or first charge succeeded: extend or activate access.
if (event.event === 'payment.updated' && event.data.status === 'succeeded') {
  const subscriptionId = event.data.metadata?.subscription_id;
  if (subscriptionId) {
    await extendAccess(subscriptionId);
    await sendReceipt(event.data);
  }
}

// A charge failed: the subscription is heading for dunning.
if (event.event === 'payment.updated' && event.data.status === 'failed') {
  const subscriptionId = event.data.metadata?.subscription_id;
  if (subscriptionId) {
    await warnCustomer(subscriptionId);
  }
}
```

Cancellations and pauses are not delivered at all — read them from
`GET /v1/subscriptions/{id}` or act on them where you initiate them.
