...
| `reference` | string | | Your internal order ID or reference for the transaction. |

<Callout type="tip">
  **Webhook Routing**: For payments created via Payment Links, Reevit automatically includes the required `org_id` and `connection_id` in the metadata. The `payment_id` is also automatically set to the unique payment ID generated for the transaction.
</Callout>
