...
2.  **Fulfill on Webhooks**: Never fulfill an order based solely on a frontend redirect. Wait for the `payment.updated` [webhook event](/docs/reevit/webhooks) carrying `data.status: "succeeded"` — there is no `payment.succeeded` webhook.
