...
const payment = await reevit.payments.createIntent({
  ...
  metadata: {              // Required: Custom metadata
    org_id: 'your_org_id',   // Required for webhook routing
    connection_id: 'your_conn_id', // Required for webhook routing
    payment_id: 'order_456', // Required for Stripe & routing
...
For inbound webhooks (Paystack, Stripe, etc.) to route correctly back to Reevit, you must include these fields in the `metadata` object:

| Field | Required | Description |
|-------|----------|-------------|
| `org_id` | ✅ Yes | Your organization ID (from Dashboard) |
| `connection_id` | ✅ Yes | The connection ID for the provider |
| `payment_id` | ✅ Yes | Your internal order ID (Required for Stripe) |
