...
The component finalizes the session with the shopper's last selected method and
provider before opening the PSP. Retries with the same selection reuse the same
intent, while changing the selection creates a separately scoped intent.
