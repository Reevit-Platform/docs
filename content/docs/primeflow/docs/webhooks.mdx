...
Webhooks are HTTP callbacks that notify your application in real-time when payment events occur. Instead of constantly polling the API to check payment status, Primeflow pushes events to your server as they happen.
...
Primeflow handles **two types of webhooks**. Understanding the difference is crucial:

### Inbound Webhooks (PSP → Primeflow)

These are webhooks from payment providers (Paystack, Flutterwave, M-Pesa, etc.) **to Primeflow**.
...
**Key Points:**
- **Primeflow handles these automatically** - you don't need to do anything
- Configure the Primeflow webhook URL in each PSP's dashboard
- Primeflow processes provider events and updates payment status

**PSP Webhook URLs:**
| Provider | Webhook URL to configure in PSP Dashboard |
|----------|-------------------------------------------|
| Paystack | `https://api.primeflow.com/webhooks/paystack` |
| Flutterwave | `https://api.primeflow.com/webhooks/flutterwave` |
| Hubtel | `https://api.primeflow.com/webhooks/hubtel` |
| M-Pesa | `https://api.primeflow.com/webhooks/mpesa` |
| Stripe | `https://api.primeflow.com/webhooks/stripe` |

<Callout type="info">
You configure these URLs in the **payment provider's dashboard** (e.g., Paystack Dashboard), NOT in Primeflow.
</Callout>

### Outbound Webhooks (Primeflow → Your App)

These are webhooks **from Primeflow to your application**.
...
- Configure your webhook URL in the **Primeflow Dashboard**
- Primeflow sends you normalized events (same format regardless of PSP)
- You verify signatures using your organization's signing secret

<Callout type="warning">
This is the webhook you configure in **Primeflow Dashboard > Developers > Webhooks**.
</Callout>
...
```
1. Payment Event Occurs → 2. Primeflow Sends Webhook → 3. Your Server Processes → 4. You Respond
```

1. **Event Occurs**: Payment succeeds, fails, or changes status
2. **Primeflow Sends**: POST request to your webhook URL with event data
...
- **Signatures**: Cryptographic signatures to verify events are from Primeflow
...
1. Go to **Settings → Webhooks** in your Primeflow dashboard
...
```bash
curl -X POST https://api.primeflow.com/v1/webhooks/config \
  -H "X-Primeflow-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123" \
  -H "Idempotency-Key: webhook-config-001" \
  -d '{
    "url": "https://yourapp.com/webhooks/primeflow",
    "events": [
      "payment.succeeded",
      "payment.failed",
      "payment.refunded",
...
```bash
curl https://api.primeflow.com/v1/webhooks/config \
  -H "X-Primeflow-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123"
```
...
Primeflow sends different events for different scenarios. Subscribe only to events you need.
...
**Always verify webhook signatures**. This ensures events are actually from Primeflow and haven't been tampered with.
...
Primeflow signs each webhook using HMAC-SHA256 with your organization's signing secret:

1. **Header**: `X-Primeflow-Signature: sha256=<hex-signature>`
...
1. Go to **Primeflow Dashboard > Developers > Webhooks**
2. Add your webhook endpoint URL
3. Click the **copy button** next to "Signing Secret"
4. Add to your environment: `PRIMEFLOW_WEBHOOK_SECRET=whsec_xxx...`
...
```typescript
// app/api/webhooks/primeflow/route.ts
...
export async function POST(request: NextRequest) {
  ...
  const signature = request.headers.get("x-primeflow-signature") || "";
  const secret = process.env.PRIMEFLOW_WEBHOOK_SECRET!;
...
app.post('/webhooks/primeflow', express.raw({ type: 'application/json' }), (req, res) => {
...
func webhookHandler(w http.ResponseWriter, r *http.Request) {
    ...
    secret := os.Getenv("PRIMEFLOW_WEBHOOK_SECRET")
...
def webhook():
    ...
    secret = os.environ['PRIMEFLOW_WEBHOOK_SECRET']
...
$secret = getenv('PRIMEFLOW_WEBHOOK_SECRET');
...
**Return 4xx/5xx for retry**: Only return error codes if you want Primeflow to retry:
...
```javascript
app.post('/webhooks/primeflow', async (req, res) => {
  try {
    // Verify signature
...
```bash
curl -X POST https://api.primeflow.com/v1/webhooks/test \
  -H "X-Primeflow-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123"
```
...
# https://abc123.ngrok.io/webhooks/primeflow
```
...
```bash
curl "https://api.primeflow.com/v1/webhooks/events?limit=20" \
  -H "X-Primeflow-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123"
```
...
```bash
curl https://api.primeflow.com/v1/webhooks/events/evt_abc123 \
  -H "X-Primeflow-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123"
```
...
```bash
curl -X POST https://api.primeflow.com/v1/webhooks/events/evt_abc123/replay \
  -H "X-Primeflow-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123"
```
...
```bash
curl https://api.primeflow.com/v1/webhooks/outbound \
  -H "X-Primeflow-Key: pfk_live_xxx" \
  -H "X-Org-Id: org_123"
```
...
Primeflow automatically retries failed webhook deliveries with exponential backoff:
...
3. **Check Firewall**: Ensure firewall allows Primeflow IPs
